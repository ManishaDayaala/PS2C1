...
MAINFOLDER = r"D:\APPdata_PS2C1"

# Create other paths relative to the main folder
training_file_path = os.path.join(MAINFOLDER, "Training", "Training_PS2C1.xlsx")  # FIXED TRAINING DATA
test_file_path = os.path.join(MAINFOLDER, "24hrData", "Dailydata_PS2C1.xlsx")  # DAILY DATA
excel_file_path = os.path.join(MAINFOLDER, "Breakdownrecords_PS2C1.xlsx")  # Recording excel for BD
folderpath = os.path.join(MAINFOLDER, "TemporaryData")  # Temporary dump files collector
threshold_file_path = os.path.join(MAINFOLDER,"Thresholds_PS2C1.xlsx") #
...
def process_data():
    ...
    assets_list = [
        "##PS-2 Primer Booth Exh Blower-1 MDE","##PS-2 Primer Booth Exh Blower-1 MNDE","**PS-2 Top Coat Booth Exh Blower-2 MDE",
        "**PS-2 Top Coat Booth Exh Blower-2 MNDE","#PS-2 Primer Booth Exh Blower-2 MDE","#PS-2 Primer Booth Exh Blower-2 MNDE ", 
        "*PS-2 Top Coat Booth Exh Blower-1 MDE ",
        "*PS-2 Top Coat Booth Exh Blower-1 MNDE", "PS-2 Primer Booth AHU Fan-NDE", "PS-2 Primer Booth AHU Motor DE", 
        "PS-2 Primer Booth AHU Motor-NDE","PS-2 Primer Booth AHU-Fan DE"]
...
def train_ensemble_model(training_file_path, model_folder_path):
    ...
    def preprocess_data(X, y):
        ...
        # Save the scaler
        joblib.dump(scaler, os.path.join(model_folder_path, "scaler_PS2C1.pkl"))
    ...
    joblib.dump(ensemble_model, os.path.join(model_folder_path, "ensemble_PS2C1.pkl"))
...
def predict_ensemble(test_file_path, model_folder_path):
    ...
    def preprocess_test_data(X_test):
        scaler = joblib.load(os.path.join(model_folder_path, "scaler_PS2C1.pkl"))
        X_test_scaled = scaler.transform(X_test)
        return X_test_scaled

    def predict(X_test_scaled):
        nn_model = joblib.load(os.path.join(model_folder_path, "ensemble_PS2C1.pkl"))
